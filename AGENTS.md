...
Репозиторий: **ibm-cargo** (`TikhonBaruch/Ibm-cargo`) — самостоятельный продукт.  
Next-приложение в **корне**; AI HTTP-матрица в **`matrix/`**. Папок `taurus/` / `llm/` нет.
...
| AI / цепочки / matrix↔compose | [`plan-ai-chains-1-2-3.md`](docs/knowledge/plan-ai-chains-1-2-3.md) · [`environments.md`](docs/knowledge/environments.md) · `npm run sync:ai-matrix` |
...
| `npm run sync:ai-matrix[:check]` | `matrix/` → `containers/{llm,ocr}` |
