...
import { rejectManufacturerRequest } from "@/lib/ved/sku-order";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const proxied = await proxyDomainApi(`/v1/manufacturer/order-requests/${id}/reject`, {
    method: "POST",
    userId,
    role,
    body,
  ...
  try {
    ...
    return NextResponse.json(await rejectManufacturerRequest(prisma, company.id, id, body));
  } catch (e) {
