...
| **Card-Enrich** | `tnved_enrich_snapshots`, `tnved_enrich_facts` (overlay) | `tnved:card-enrich` load/reconcile · `cardEnrich` на GET :code · **не** пишет `notes` · [`plan-tnved-card-enrich.md`](./plan-tnved-card-enrich.md) |
...
HTTP: `GET /v1/tnved/search` (titleRu **или** notes **или** prefix кода; `heading=1` — 4-значные товарные позиции главы; ответ `{ items, total }`), `GET /v1/tnved/:code` (**карточка:** предки, **дети** `parentCode` ≤16, **related** overlay C20, ЕТТ `rate` или `null`, НДС/сбор-hint), `POST /v1/tnved/import` (admin, ≤500).  
