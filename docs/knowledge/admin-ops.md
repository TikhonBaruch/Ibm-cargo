...
- **Два слоя ТН ВЭД** ([`data-model.md`](./data-model.md) §2.1): **corpus** `matrix/data/tnved/normalized/codes.jsonl` → `containers/llm` classify (lookup-v1); **Prisma** `TnvedCode` → broker autocomplete + admin import. Compose mount: `./matrix/data/tnved/normalized:/data/tnved:ro` + `TNVED_CODES_PATH`. Не смешивать runtime corpus с DB import.
