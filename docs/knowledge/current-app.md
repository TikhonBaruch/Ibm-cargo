...
- **Live chrome lbm-bro:** `/cabinet` 5 тайлов + поиск/колокол + superapp; `/cabinet/new` = копия шага «Что ввозите?» (C10) + клик «Мультипозиция» (C11, live D10 caps) + панель «Уточняем для точности кода» на single (C12, lab heuristic); inner panes C4 = `.card` / `table.data` / `.field` / `.chat-box`; `DesignerStub` не рисует бейдж (C9); chrome производителя скрыт (C6) · `/broker` `/admin` ops-shell · lab `/client` референс · [`plan-lbm-bro-visual.md`](./plan-lbm-bro-visual.md)
