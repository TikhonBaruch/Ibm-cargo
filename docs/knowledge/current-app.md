...
| Полный каталог ТН ВЭД | **C18:** lab-классификатор (~13 123 узлов + алиасы в `notes`) upsert в sweb `newlsu_lbm` (`tnved:load -- --lab`), без delete rates. **Не** `tnved:load -- --full` / TWS dump на sweb. Local TWS fill — [`plan-tnved-collect.md`](./plan-tnved-collect.md) | [`plan-lbm-bro-tnved-catalog.md`](./plan-lbm-bro-tnved-catalog.md) · [`plan-tnved-opendata-card.md`](./plan-tnved-opendata-card.md) |
