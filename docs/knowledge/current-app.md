...
- **Live chrome lbm-bro:** `/cabinet` 5 тайлов + поиск/колокол + superapp; inner panes C4 = `.card` / `table.data` / `.field` / `.chat-box`; hold → `DesignerStub` · `/broker` `/admin` ops-shell · lab `/client` референс · [`plan-lbm-bro-visual.md`](./plan-lbm-bro-visual.md)
