...
- **Live chrome lbm-bro:** `/cabinet` product-shell + superapp + карточки заявок / wizard chrome / stepper; `/broker` `/admin` ops-shell · hold → `DesignerStub` · lab `/client` референс · [`plan-lbm-bro-visual.md`](./plan-lbm-bro-visual.md)
