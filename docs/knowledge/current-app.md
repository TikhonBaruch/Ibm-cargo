...
| **CSV/XLSX/PDF/image import preview (API + UI)** | `POST /api/v1/imports/products/preview` (`csv`/`xlsxBase64`/`pdfBase64`/`imageBase64`) + pack dropzone на `/cabinet/new`; create через `/calculations`; лимит D10; пустое фото → 200 fail-open | `smoke:csv-import` · `smoke:pdf-import` · unit `product-import.test.ts` · `invoice-vision-table.test.ts` |
...
| Полный каталог ТН ВЭД | **C18–C32 + C35a–e + morph H1–H5 на main**. **Coverage P0–P6** on main: **53 packs**. **Cov-P0…P12 stack** (PR #50–#56): **78 packs** + cascade S+ + staging §Cov — pending human merge. **Precision P0–P7** (#38–#42). Plan [`plan-hint-coverage-p0.md`](./plan-hint-coverage-p0.md) · [`plan-hint-coverage-expansion.md`](./plan-hint-coverage-expansion.md) · [`plan-hint-chains-precision-audit.md`](./plan-hint-chains-precision-audit.md). Prod https://ibm-cargo-phi.vercel.app. | [`plan-lbm-bro-tnved-catalog.md`](./plan-lbm-bro-tnved-catalog.md) · [`plan-classify-cascade-c23.md`](./plan-classify-cascade-c23.md) · [`plan-c32-preview-devex.md`](./plan-c32-preview-devex.md) |
