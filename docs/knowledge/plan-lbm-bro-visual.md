...
`/cabinet/tnved` повторяет chrome `/client/tnved`: две колонки, автовыбор первого хита, правая карточка (pill / `.tnved-code` / metric-row / notes / риск / CTA). Источник — live `GET /api/v1/tnved/*`, НДС **22%**, сбор ПП 1637. CTA ведёт на `/cabinet/new?hs=&desc=`. Не копировать `tnved.json`, freemium-peek и «НДС 20%». План: [`plan-lbm-bro-tnved-dir.md`](./plan-lbm-bro-tnved-dir.md).
