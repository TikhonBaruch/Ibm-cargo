...
Связано: перевозка / 3PL — [`growth.md`](./growth.md) §«Перевозка»; внешняя LLM-матрица logistics — репозиторий [`llm`](../../matrix/) (`services/logistics`).
...
Инвентарь-зеркало для LLM-матрицы (тот же ibm-cargo): [`../../matrix/docs/sources-incoterms.md`](../../matrix/docs/sources-incoterms.md).
