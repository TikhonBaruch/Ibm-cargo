...
- `npm warn allow-scripts … not yet covered by allowScripts` — advisory npm 11.16+. В `package.json` `allowScripts`: имя + pin Prisma 6.19.3, sharp **0.35.3** (lockfile) и **0.34.5** (старый лог), tesseract.js, unrs-resolver. Не `ignore-scripts`. На npm 10 предупреждения нет.
