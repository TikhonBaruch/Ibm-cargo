...
Offline engines for skip: `precedent-v1` \| `precedent-v2` \| `cascade-v1` (heuristic alone **не** skip). Tag `aiDraft.skipReason`: `offline-hit:…` \| `llm-low-conf:…` \| `llm-miss`. Next create: early `tryPrecedentDraft` hit also tags `offline-hit` (parity with `containers/api`). Ops count: `npm run ops:precedent-count` ([`plan-precedent-bulk.md`](./plan-precedent-bulk.md) §Ops). **C35e:** must-cover fixture assert offline-hit ≥ 60% (`test:classify-cascade`). **A4:** без `DEEPSEEK_API_KEY` → create `AI_READY` (`smoke:mvp`). Канон: [`plan-c35-offline-first-hs.md`](./plan-c35-offline-first-hs.md).
