...
Канон: [`plan-tnved-opendata-card.md`](./plan-tnved-opendata-card.md) · [`ai-pipeline.md`](./ai-pipeline.md) · [`customs-payments.md`](./customs-payments.md) · llm [`sources-tnved.md`](../../matrix/docs/sources-tnved.md).
...
Что лежит в `scripts/data/tnved/` и `matrix/data/tnved/` (jsonl, TWS, PDF). Поиск TWS CSV в `~/Backups`, если пропал с диска.
