...
| Compose LLM enrich | profile `scale`/`full`: service `llm` :4500; mount `./matrix/data/tnved/normalized:/data/tnved:ro`; `LLM_SERVICE_URL=http://llm:4500` in ai/api/web (override host `.env` 127.0.0.1); gate `llmEnrichEnabled` |
