...
| Шаг | Что | Done when |
|-----|-----|-----------|
| M0a | Зафиксировать surface = mobile web shell на client routes | ADR-lite строка / этот план §3.1 |
| M0b | Inventory screens ↔ panes (`cabinets/client`) | таблица §3.2 |
| M0c | Token map: `--blue`, `--bg`, `--radius`, Manrope/Nunito, `.go-*` / `.wiz-*` / `.cl-*` | checklist в PR |
| M0d | Tabbar wireframe → CSS classes (не копировать SMS-auth) | mock в lab или CSS-only sketch |
| M0e | Out-of-scope list подписан | §7 |
...
| Шаг | Что | API / UI |
|-----|-----|----------|
| M1a | Mobile-first login screen в токенах lbm-bro (не обязательно `LandingAuthShell` pixel) | `app/login` adapt **или** `/cabinet`-entry gate |
| M1b | Email + password → NextAuth credentials | existing `signIn` |
| M1c | После login: `GET /api/v1/me` → role CLIENT → home | redirect `/cabinet` |
| M1d | Logout + session expire UX | clear session → login |
| M1e | Demo hint (non-prod): `client@example.com` / `demo1234` | как web |
| M1f | Register hold или тонкий link на `/register` | не блокирует M1 |
...
| Шаг | Что | Паттерн |
|-----|-----|---------|
| M2a | Tabbar sticky bottom (Главная · Заявки · +Новый · Чат · Компания) | wireframe IA + lbm tokens |
| M2b | `.go-dash` greet + hero tile «Новый просчёт» | `ClientSuperappHome` |
| M2c | Лента активных заявок / filter chips | `.go-feed` / `.filter-chips` |
| M2d | Скрыть shipping/factory/commercial invoice | honest skin C6/C8 |
...
**Сейчас:** план принят → следующий код-срез **M0d/M1a** (mobile login + tabbar на токенах lbm-bro) отдельным PR.  
**Не начинать** M3 pay / M6 native без зелёного M1–M2.  
**Стек:** M2+ лучше ветвить от `cursor/lbm-bro-honest-skin-e1f0` / PR #19 после/параллельно skin; docs-план живёт от `main`.

Параллельно: ТН ВЭД / C31 — [`plan-next-vector-c28.md`](./plan-next-vector-c28.md) (на feature-ветке; файла может не быть на `main` до merge).
