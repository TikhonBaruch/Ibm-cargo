...
1. Секреты в **`.env`**: выделенная `DATABASE_URL` + `S3_*` (bucket `lbm`) + `NEXTAUTH_*`.
2. Опционально **`.env.local`**: только localhost UI / `ALLOW_MOCK_TOPUP` / mesh URLs — **не** переопределять `DATABASE_URL` / `S3_*` (см. `.env.local.example`).
...
Карта слоёв: [`docs/architecture-map.md`](../architecture-map.md).
...
1. `docker compose --profile core up -d` (project name **`lbm`**).  
2. Postgres/API в сети compose (`lbm`).  
3. Mode A UI по-прежнему берёт **`DATABASE_URL` / `S3_*` из `.env`** (выделенная БД + bucket `lbm`). В `.env.local` можно указать только `AI/LLM/OCR_SERVICE_URL` на `127.0.0.1`, не подменяя БД/S3.  
