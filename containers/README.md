...
**AI matrix canon (D35):** HTTP classify/OCR живут в sibling-репо `matrix`. Синхронизация зеркал: `npm run sync:ai-matrix`. Сборка compose напрямую из matrix: `LLM_DOCKER_CONTEXT=./matrix/services/classification`, `OCR_DOCKER_CONTEXT=./matrix/services/ocr`. **Model ≠ container** — см. [`docs/knowledge/plan-parallel-ownership.md`](../docs/knowledge/plan-parallel-ownership.md).
