#!/usr/bin/env node
/**
 * Sync AI capability mirrors: matrix/ → containers/{llm,ocr}.
 * Canon: matrix/services/{classification,ocr}
 * Usage:
 *   node scripts/sync-ai-matrix.cjs           # copy
 *   node scripts/sync-ai-matrix.cjs --check   # exit 2 if drift (when ./matrix exists)
...
const matrixRoot = path.resolve(root, "matrix");
...
if (!fs.existsSync(matrixRoot)) {
  if (checkOnly) {
    console.log("sync-ai-matrix: ./matrix missing — skip check");
    process.exit(0);
  }
  console.error("sync-ai-matrix: expected matrix package at", matrixRoot);
  process.exit(1);
}
