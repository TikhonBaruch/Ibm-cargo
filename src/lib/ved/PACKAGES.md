...
HTTP classify/OCR/duty живут в пакете **`matrix`** того же репозитория ibm-cargo (`services/classification`, `services/ocr`).  
`containers/llm` / `containers/ocr` — Compose **mirror**; sync: `npm run sync:ai-matrix`.  
