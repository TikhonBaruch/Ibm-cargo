...
describe("VED access — resolvePathAccess", () => {
  ...
  it("allows CLIENT on /client UI lab", () => {
    expect(resolvePathAccess("CLIENT", "/client/new")).toEqual({ type: "allow" });
  });

  it("redirects BROKER from /client to /broker", () => {
    expect(resolvePathAccess("BROKER", "/client")).toEqual({ type: "redirect", to: "/broker" });
  });
...
describe("VED access — protected mutations inventory", () => {
  ...
  it("does not mark unrelated GETs as protected mutations", () => {
    ...
    expect(matchesProtectedMutation("GET", "/api/v1/imports/products/describe")).toBe(false);
  });
});
