...
1. **`app/.env`**: `DATABASE_URL` (выделенная БД продукта), `NEXTAUTH_*`, `S3_*` (bucket `lbm`), optional `PAYMENTS_SERVICE_URL`.
2. Опционально **`app/.env.local`**: localhost UI / `ALLOW_MOCK_TOPUP` — **не** override `DATABASE_URL` / `S3_*` (шаблон `app/.env.local.example`).
...
Карта слоёв / lab: [`docs/architecture-map.md`](../../../docs/architecture-map.md).
...
| Compose DB (Mode B) | `api`/`web`/`worker` → in-network `postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB}` (defaults в `docker.env.example`). Host Mode A Next остаётся на **`app/.env` DATABASE_URL** (выделенная БД продукта) |
