...
import { approveManufacturerProposal } from "@/lib/ved/manufacturer-directory";
...
export async function POST(
  ...
  const proxied = await proxyDomainApi(`/v1/admin/manufacturer-proposals/${id}/approve`, {
    method: "POST",
    userId: user.id!,
    role: user.role,
    body: {},
  ...
  try {
    const result = await approveManufacturerProposal(prisma, {
      proposalId: id,
      actorUserId: user.id!,
    });
