...
import { acceptManufacturerRequest } from "@/lib/ved/sku-order";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const proxied = await proxyDomainApi(`/v1/manufacturer/order-requests/${id}/accept`, {
    method: "POST",
    userId,
    role,
    body,
  ...
  try {
    ...
    return NextResponse.json(await acceptManufacturerRequest(prisma, company.id, id, body));
  } catch (e) {
