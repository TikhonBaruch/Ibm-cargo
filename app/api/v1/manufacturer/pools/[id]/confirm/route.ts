...
import { confirmManufacturerPool } from "@/lib/ved/sku-order";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const proxied = await proxyDomainApi(`/v1/manufacturer/pools/${id}/confirm`, {
    method: "POST",
    userId,
    role,
    body: {},
  ...
  try {
    ...
    return NextResponse.json(await confirmManufacturerPool(prisma, company.id, id));
  } catch (e) {
