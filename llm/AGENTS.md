...
HTTP-матрица AI-сервисов для пакета **app** (`../taurus`) в репозитории **ibm-cargo**.  
App владеет Calculation FSM (D8), pay gate (D11), ledger, cabinets.  
Этот каталог владеет **только model HTTP APIs**.

ibm-cargo развивается **отдельно** от upstream taurus; пути `/home/andrey/*` не канон.
...
5. Keep **classification** `/v1/classify` + `/v1/duty` compatible with `taurus/docs/contracts/d-draft.llm.json` / `LLM_SERVICE_URL`.
6. Keep **ocr** `/v1/extract` compatible with `taurus/docs/contracts/d-ocr.ai.json` / `OCR_SERVICE_URL`.
...
| `docs/` | Matrix, vision, app integration; `sources-tnved.md` / `sources-incoterms.md` / `sources-payments.md` |
| `reference/` | Read-only copies of app adapters |

## Sync with app (`../taurus`)

Этот каталог — **канон** classify/OCR HTTP. `taurus/containers/llm` и `taurus/containers/ocr` — Compose mirrors.

```bash
# From taurus/:
npm run sync:ai-matrix          # copy ../llm → containers/{llm,ocr}
...
Changing an envelope here → update `taurus/docs/contracts` (and dual-path callers if needed).  
See `docs/integration-taurus.md`, `docs/sources.md`, ADR **D35** / `plan-parallel-ownership.md`.
